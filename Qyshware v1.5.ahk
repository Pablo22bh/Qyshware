...
IfNotExist, off.ini
{
	IniWrite, 1, off.ini, Offsets, Aimbot
	IniWrite, 1, off.ini, Offsets, Triggerbot
	IniWrite, 1, off.ini, Offsets, All
}
IfExist, off.ini
{
	IniRead, inioffaim, off.ini, Offsets, Aimbot
	IniRead, iniofftrig, off.ini, Offsets, Triggerbot
	IniRead, inioffall, off.ini, Offsets, All
}
IfNotExist, pos.ini
{
	MsgBox, pos.ini not found!
	ExitApp
}
IfExist, pos.ini
{
	IniRead, isa1x, pos.ini, First, X
	IniRead, isa1y, pos.ini, First, Y
	IniRead, isa2x, pos.ini, Second, X
	IniRead, isa2y, pos.ini, Second, Y
	IniRead, iw, pos.ini, Size, Width
	IniRead, ih, pos.ini, Size, Height
}
If(A_IsAdmin)
{
    ...
    URLDownloadToFile, https://raw.githubusercontent.com/MoHatKnock/Qyshware/master/beta.txt, C:\Users\%A_UserName%\check.txt
...
if(ab_h==1 and ab_active==1)
{
	...
	{
		PixelSearch, x, y, %isa1x%, %isa1y%, %isa2x%, %isa2y%, 0x1515FF, 30, Fast
		if(y!=null)
		{
			mid := (iw / 2) + isa1x + midpos
			midp := mid + apmslider
			midm := mid - apmslider
			math := ab_s
			if(x>=midp)
...
toggletrigger:
Gui, submit, nohide
midt := (iw / 2) + isa1x + midpos
midtp := midt + tpmslider
midtm := midt - tpmslider
if(b_t==0 and b_h==1 and b_active)
{
	while(GetKeyState(Ini_b_k, "p"))
	{
		PixelSearch, xt, yt, %isa1x%, %isa1y%, %isa2x%, %isa2y%, 0x1515FF, 30, Fast
		Gui, submit, nohide
		if(xt<midtp and xt>midtm)
		{
			Click
			Sleep, 200
		}
...
else if(b_t==1 and b_h == 0 and b_active)
{
...
Gui, Add, GroupBox, x642 y179 w170 h80 ,
Gui, Add, Text, x10 y291 w35 h20 , Aimbot:
Gui, Add, Slider, x50 y289 w170 h20 +ToolTip AltSubmit gUpdate vapmslider Range0-50, %inioffaim%
Gui, Add, Text, x260 y291 w35 h20 , PosX:
Gui, Add, Slider, x290 y289 w170 h20 +ToolTip AltSubmit gUpdate vmidpos Range-100-100, %inioffall%
Gui, Add, Text, x490 y291 w35 h20 , Trigger:
Gui, Add, Slider, x530 y289 w170 h20 +ToolTip AltSubmit gUpdate vtpmslider Range0-50, %iniofftrig%
Gui, Add, Checkbox, x750 y289 vltcb glt, Show areas
Gui, Font, s13 q5
Gui, Add, Text, x652 y201 w150 h40 , Private Edition v%cversion%`n by. Qysh
Gui, Show, w834 h320, CS:GO - Qyshware
...
area1y := isa1y
Gui, 2:destroy
Gui, 2:Color, 0x00ff00
Gui, 2: -Caption +E0x80000 +LastFound +OwnDialogs +Owner +AlwaysOnTop +E0x20
gui, 2:margin,, 0
Gui, 2:Show, x%area1y% y%area1y% w2 h2 NoActivate, uj7557j7j67j67j67j7
WinSet, TransColor, c00ff00 100, uj7557j7j67j67j67j7
Gui, 2:Color, 0xff0000
Gui, submit, nohide
Gui, 3:destroy
Gui, 3:Color, 0x00ff00
Gui, 3: -Caption +E0x80000 +LastFound +OwnDialogs +Owner +AlwaysOnTop +E0x20
gui, 3:margin,, 0
Gui, 3:Show, x%area1y% y%area1y% w2 h2 NoActivate, 456ztrf45tz6hfdy4wrrz6462
WinSet, TransColor, c00ff00 100, 456ztrf45tz6hfdy4wrrz6462
Gui, 3:Color, 0xffffff
Gui, 4:destroy
Gui, 4:Color, 0x0000ff
Gui, 4: -Caption +E0x80000 +LastFound +OwnDialogs +Owner +AlwaysOnTop +E0x20
gui, 4:margin,, 0
linex := isa1x + (iw / 2)
Gui, 4:Show, x%linex% y%isa1y% w2 h80 NoActivate, fviokefjnmgioefnmjgo
WinSet, TransColor, c0000ff 100, fviokefjnmgioefnmjgo
Gui, 4:Color, 0x00ff00
Loop
{
	Gui, submit, nohide
	width := apmslider * 2
	width2 := tpmslider * 2
	area1x := isa1x - apmslider + (iw / 2) + midpos
	area2x := isa1x - tpmslider + (iw / 2) + midpos
	linex := (iw / 2) + isa1x + midpos - 1
	ally := isa1y - 5
	WinMove, uj7557j7j67j67j67j7,, %area1x%, %ally%, %width%
	WinMove, 456ztrf45tz6hfdy4wrrz6462,, %area2x%, %ally%, %width2%
	WinMove, fviokefjnmgioefnmjgo,, %linex%, %ally%
	if(ltcb==0)
	{
		Gui, 2:destroy
		Gui, 3:destroy
...
Exit:
Gui, submit, nohide
	IniWrite, %apmslider%, off.ini, Offsets, Aimbot
	IniWrite, %midpos%, off.ini, Offsets, All
	IniWrite, %tpmslider%, off.ini, Offsets, Triggerbot
ExitApp
return